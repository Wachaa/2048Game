...
// Normalize each top player’s profile_picture into a public URL:
foreach ($topPlayers as &$player) {
    $rawPfp = $player['profile_picture'] ?? '';
    if ($rawPfp && file_exists($rawPfp)) {
        // Strip off DOCUMENT_ROOT (e.g. "/var/www/html")
        $player['profile_picture'] = substr($rawPfp, strlen($_SERVER['DOCUMENT_ROOT']));
    } else {
        // Fallback to default avatar
        $player['profile_picture'] = '/uploads/user.png';
    }
}
unset($player);
...
while ($row = $result->fetch_assoc()):
    ...
    $rawPfp = $row['profile_picture'] ?? '';
    if ($rawPfp && file_exists($rawPfp)) {
        $row['profile_picture'] = substr($rawPfp, strlen($_SERVER['DOCUMENT_ROOT']));
    } else {
        $row['profile_picture'] = '/uploads/user.png';
    }
    ...
    if ($highlightUser && $row['username'] === $username) {
        $highlightClass = 'bg-yellow-200 font-bold';
    }
...
<tr class="<?php echo $highlightClass; ?>">
  ...
  <img 
    ...
    class="w-8 h-8 rounded-md border-2 border-black object-cover" 
  />
</td>
