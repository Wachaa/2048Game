...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profile_picture'])) {
    // $uploadDir: server‐side directory where we save uploaded files
    ...
    if (in_array($ext, $allowed)) {
        ...
        $targetPath = $uploadDir . $newName; // e.g. /var/www/html/uploads/1747384696_myimage.png

        if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $targetPath)) {
            // Store the absolute, server‐side path in the database:
            // We can use $_SERVER['DOCUMENT_ROOT'] to get "/var/www/html"
            // and append "/uploads/$newName".
            $absolutePath = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/uploads/' . $newName;
            $stmt         = $conn->prepare("UPDATE users SET profile_picture = ? WHERE id = ?");
            $stmt->bind_param("si", $absolutePath, $userId);
            $stmt->execute();
        }
    }
...
if ($raw && file_exists($raw)) {
    // If $raw holds an absolute path like "/var/www/html/uploads/xyz.png",
    // strip off DOCUMENT_ROOT so the browser sees "/uploads/xyz.png"
    $profileUrl = substr($raw, strlen($_SERVER['DOCUMENT_ROOT']));
} else {
    // No custom avatar on disk → use your default avatar file
...
// If submitted, preserve username/password fields
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      <form id="avatarForm" method="POST" enctype="multipart/form-data" class="flex justify-center mb-6">
        <div class="relative w-20 h-20 border-2 border-black rounded-md overflow-hidden">
          <label for="profile_picture" class="cursor-pointer">
            <img
              src="<?= htmlspecialchars($profileUrl) ?>"
              alt="Avatar"
      ...
      <form action="" method="POST" class="flex flex-col gap-4">
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="togglePassword absolute right-2 top-1/2 transform -translate-y-1/2 text-sm text-blue-600 hover:text-blue-800"
        ...
        <button
          type="submit"
          class="mt-4 bg-black text-white py-2 rounded hover:bg-gray-800 transition duration-200"
        >
          Save Changes
        </button>
      </form>
